...
//only one file descriptor as input 
int myencryption(int fd)
{
unsigned char palaksKey[16] = {'q','w','e','r','t','y','u','i','o','p','a','s','d','f','g','k'};
//unsigned char palaksKey[16] = "qwertyuiopasdfgk"; doesn't work 
unsigned char newkey[16];
int len = lseek(fd, 0, SEEK_END);
unsigned char* fdb;

fdb = (unsigned char*) calloc(len, sizeof(char));
pread(fd, fdb, len, 0);
string fdSTR(reinterpret_cast<char*>(fdb), len);
char const* fdC = fdSTR.c_str();

	
int datalength = fdSTR.size();
unsigned char *buf = (unsigned char*)malloc(datalength + 1);
memset(buf, 0, len + 1);
...
RC4(&key, datalength, (const unsigned char*)fdC, buf);

string out((char*)buf, len);
const char *outbuf = out.c_str();
pwrite(fd, outbuf, out.length(), 0);
ftruncate(fd, out.length());
return 0;
}
...
myencryption(fd);
ofstream test;
test.open("//home//palak2//Desktop//newlog.txt",ofstream::out | ofstream::app);
test << "inside load looking for this " << endl;
test.close();
...
//test number 1 this is the only thing that runs on upload 
...
test.open("//home//palak2//Desktop//newlog.txt",ofstream::out | ofstream::app);
test << "inside read under reading" << endl;
test << bytes << endl;
test.close();
...
test.open("//home//palak2//Desktop//newlog.txt",ofstream::out | ofstream::app);
test << "inside reading number 3" << endl;
test << bytes << endl;
test.close();
